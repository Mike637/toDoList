...
import "./ToDo.css";
/* Function generates special id for each task */
const generateId = () =>
...
/* Function Add task to the todo list*/ 
  const addTask = () => {
    ...
    setToDoList((prev) => [
      ...prev,
      { id: generateId(), name: textInput.current.value },
    ]);
    setVal("");
  };
/* Function Add task to the todo list using click Enter*/ 
...
/* Function edit task and update todo list*/ 
      ...
      <div className="main__container">
        <div className="main__addTask addTask">
          ...
          <input
            ...
            onKeyDown={addTaskEnterClick}
            placeholder="Type here..."
          />
          <button className="addTask__button" onClick={addTask}>
            Add
          </button>
        </div>
          ...
          {toDoList.length === 0 ? (
            ...
            toDoList.map((item, index, array) => (
              <TaskChangeDelete
                ...
                id={item.id}
                onDelete={(id) =>
                  setToDoList(array.filter((task) => task.id !== id))
                }
